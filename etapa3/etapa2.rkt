...
(define (get-suffixes text)
  (if (empty? (cdr text)) '((#\$)) (cons text (get-suffixes (cdr text)))))
...
(define (ast-func suffixes)
  (let ((first-ch (caar suffixes))) ; primul ch al primului elem din suffixes
    (cons (list first-ch)
          (map cdr suffixes))))
...
(define (cst-func suffixes)
  (let* ((cst-label (longest-common-prefix-of-list suffixes)))
    (cons cst-label
          (map (λ (suffix) (drop suffix (length cst-label))) ; elimin prefixul comun
               suffixes))))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (define (build-tree suffixes)
    (if (empty? alphabet)
        '()
        (let* ((ch-alph-suff (map (λ (ch) (get-ch-words suffixes ch)) alphabet))
               (non-empty-words (filter (λ (x) (not (null? x))) ch-alph-suff)))
          (map (λ (ch)
  ...
  (lambda (labeling-func)
    (lambda (text)
      (let* ((nt (append text (list #\$)))
             ...
             (alphabet (sort (remove-duplicates nt) char<?))) ; ordine lexicografica
