...
(define (longest-common-prefix-of-list words)
 (define (longest-common-prefix-list-helper prefix words)
    (cond
      ((or (null? (car words)) (null? (cdr words)))
       (reverse prefix))
      (else
       (let* ((first-word (car words))
              (current-prefix (longest-common-prefix (car words) (cadr words))))
         (longest-common-prefix-list-helper (reverse current-prefix) (cdr words))))))
(car (longest-common-prefix-list-helper '() words)))
