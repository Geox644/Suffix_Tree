...
;; Constructor pentru colecția vidă
(define-syntax-rule (empty-collection) (empty-stream))

;; Funcție pentru verificarea dacă o colecție este vidă
(define-syntax-rule (collection-empty? x)
  (stream-empty? x))

;; Funcție pentru extragerea primului element dintr-o colecție
(define-syntax-rule (collection-first x)
  (stream-first x))

;; Funcție pentru extragerea restului colecției, fără primul element
(define-syntax-rule (collection-rest x)
  (stream-rest x))

(define-syntax-rule (collection-filter x xs)
  (stream-filter x xs))

(define-syntax-rule (collection-map x xs)
  (stream-map x xs))

(define-syntax-rule (collection-second x)
  (stream-first (stream-first x)))

(define-syntax-rule (collection-third x)
  (stream-first (stream-first(stream-first x))))
